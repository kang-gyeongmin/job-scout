...
def extract_json_object(text: str) -> dict:
    """응답 텍스트에서 JSON 객체({...}) 하나를 추출한다. fence 유무 모두 처리."""
    fence = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", text, re.DOTALL)
    raw = fence.group(1) if fence else text[text.find("{"):text.rfind("}") + 1]
    return json.loads(raw)
...
ENRICH_SYSTEM_PROMPT = """당신은 채용 공고 페이지를 분석하는 전문가다. 주어진
채용 공고 페이지 텍스트를 지원자 프로필과 비교해 채점한다.

규칙:
- 페이지 텍스트에서 공고 제목(title)과 회사명(company)을 추출한다.
- 프로필 대비 적합도 1~10점(score), 한 줄 이유(reason), 3줄 이내 요약(summary)을 만든다.
- 대상 경력을 벗어나는(요구 경력이 높은) 공고는 점수를 낮게 준다.
- 페이지가 채용 공고가 아니거나 정보가 부족하면 낮은 점수를 주고 요약에 그 사실을 밝힌다.

최종 응답은 반드시 아래 형식의 JSON 객체 하나만 출력한다:
```json
{"title": "...", "company": "...", "score": 7, "reason": "...", "summary": "..."}
```"""


async def score_text(url: str, page_text: str, profile_text: str,
                     config: dict) -> dict:
    """단일 공고 페이지 텍스트를 프로필과 비교해 채점한다.

    title·company·score·reason·summary를 담은 dict를 반환한다. 파싱에 실패하면
    RuntimeError를 던진다(호출부에서 '분석 실패'로 처리).
    """
    prompt = (
        f"# 지원자 프로필\n{profile_text}\n\n"
        f"- 대상 경력: {config.get('experience', '')}\n"
        f"- 희망 지역: {config.get('locations', '')}\n\n"
        f"# 공고 URL\n{url}\n\n"
        f"# 공고 페이지 텍스트\n{page_text[:8000]}"
    )
    options = ClaudeAgentOptions(system_prompt=ENRICH_SYSTEM_PROMPT, max_turns=1)
    result_text = ""
    async for message in query(prompt=prompt, options=options):
        if isinstance(message, ResultMessage):
            result_text = message.result or ""
    try:
        item = extract_json_object(result_text)
        return {
        ...
        raise RuntimeError(
            f"채점 응답 파싱 실패: {e!r} — 응답 앞부분: {result_text[:300]!r}") from e
